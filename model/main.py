...
def main():
    ...
    df_limpo = pd.read_csv('..\data\caracteres-limpo.csv', delimiter=",", encoding='UTF-8-sig')
    df_ruido = pd.read_csv('..\data\caracteres-ruido.csv', delimiter=',', encoding='UTF-8')
    df_ruido_20 = pd.read_csv('..\data\caracteres_ruido20.csv', delimiter=',', encoding='UTF-8')
    # data = df_limpo[:,0:2]
    # labels = df_limpo[:,2:3]
    # data = df_limpo[:,0:63]
    ...
    mlp_limpo.train(training_dataset, test_dataset, 1500, 0.8)
